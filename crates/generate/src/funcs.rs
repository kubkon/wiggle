...
pub fn define_func(names: &Names, func: &witx::InterfaceFunc) -> TokenStream {
    ...
    let abi_args = quote!(
            ctx: &mut #ctx_type, memory: &mut wiggle_memory::GuestMemory,
            #(#params),*
    );
    ...
    let error_handling = |location: &str| -> TokenStream {
        if let Some(tref) = &err_type {
            ...
            quote! {
                let e = wiggle_memory::GuestError::InFunc { funcname: #funcname, location: #location, err: Box::new(e) };
                let err: #err_typename = wiggle_memory::GuestErrorType::from_error(e, ctx);
    ...
    let success = if let Some(ref err_type) = err_type {
        ...
        quote! {
            let success:#err_typename = wiggle_memory::GuestErrorType::success();
